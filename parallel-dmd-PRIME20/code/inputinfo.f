...
!     just of the appropriate length for this trick
	...
	!LR: Changed a hardcoded 2-species variable reference to a noptotal variable
      	...
      	do while (.true.)
		read(7,end=120) coll,t,old_rx,old_ry,old_rz
      	enddo
...
120	continue
	close(unit=7)
      !LR: Changed a hardcoded 2-species variable reference to a noptotal variable
      	...
      	read(7) coll,old_rx,old_ry,old_rz
!LR: Changed a hardcoded 2-species variable reference to a noptotal variable
      	do k=1,(noptotal)   
         	sv(4,k)=old_rx(k)
         	sv(5,k)=old_ry(k)
...
!     	input identity is for single chain, need identity(nop)
	...
	do k=3*chnln1+1,numbeads1
      	...
      	do i=1,400
      		read(7,"(2(2x,i2,2x),f8.3)") iiii,jjjj,eptemp
      		ep(iiii,jjjj)=-eptemp
      	end do
...
711  		format(2(2x,i2,2x),2(f6.3,2x))
	...
	do i=1,20
		read(7,'(5(f6.3,2x))') sz8,sz6,sz7,sz9,sz10
		...
		do k=1,chnln1
			if(fside1(k).ne.0) then
				rx2=(sv(1,l-1+fside1(k))-sv(1,l+k-1))
            			...
            			write(73,7373) "rca ",l,k,aa(k),rca2*boxl,bdln(k) 
            			...
            			write(73,7373) "rnh ",l,k,aa(k),rnh2*boxl,bl_rn(k) 
            			...
            			write(73,7373) "rco ",l,k,aa(k),rco2*boxl,bl_rc(k) 
      		...
      		do l=1,nop2,numbeads2
        		do k=1,chnln2
        			if(fside2(k).ne.0) then
            				...
            				rz2=(rz2-dnint(rz2))**2
            				rca2=dsqrt(rx2+ry2+rz2)
            				write(73,7373) "rca ",l,k,aa(k),rca2*boxl,bdln(chnln1+k) 
            				...
            				rz2=(rz2-dnint(rz2))**2
            				rnh2=dsqrt(rx2+ry2+rz2)
            				write(73,7373) "rnh ",l,k,aa(k),rnh2*boxl,bl_rn(chnln1+k) 
            				...
            				rz2=(rz2-dnint(rz2))**2
            				rco2=dsqrt(rx2+ry2+rz2)
            				write(73,7373) "rco ",l,k,aa(k),rco2*boxl,bl_rc(chnln1+k) 
