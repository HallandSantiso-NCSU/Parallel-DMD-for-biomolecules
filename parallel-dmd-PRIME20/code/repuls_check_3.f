	subroutine repuls_check_3(i,j,k,rating)
...
#include "def.h"
	...
	if ((ncaj .ne. k).and.(d1.gt.shder_dist1)) then
		m=m+1        
	endif
	...
	d2=dsqrt(d2)
     	...
     	if ((nnjp1 .ne. k).and.(d2.gt.shder_dist2)) then   
       	m=m+1
   	endif
	...
	d3=dsqrt(d3)
    	...
    	if ((ncai .ne. k).and.(d3.gt.shder_dist3)) then  
      		m=m+1
      	endif                 
	...
	rzij=rzij-dnint(rzij)
   	...
   	if ((ncim1 .ne. k).and.(d4.gt.shder_dist4)) then   
       	m=m+1
     	endif
